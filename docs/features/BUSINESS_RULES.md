...
**자동 매칭 없음. 상신 시점에 작성자가 명시적으로 지정.**

- **품의서 작성 폼**엔 결재자 필드 없음 (작성은 단순)
- **상신 버튼 클릭 → 결재선 지정 모달**에서 3명 지정 후 상신
  - 영업담당 (기본값: 작성자 자신)
  - 팀장 (기본값: `role=TEAM_LEADER`인 첫 번째 활성 사용자)
  - CEO (기본값: `role=CEO`인 첫 번째 활성 사용자)
- **대리자 선택 범위**: `isActive=true`인 모든 사용자 (role 체크 없음 — 부재 시 다른 사람 지정 가능)
- 지정된 ID는 `SalesApproval.salesManagerId` / `teamLeaderId` / `ceoId` (영업), `MAApproval`의 동일 필드(신규 추가)에 저장
- **근거**: 소규모 조직이라 자동 매칭도 가능하지만, 매칭 대상자가 부재/퇴사 중이면 상신 자체가 불가해지는 리스크 회피
...
- **2026-04-22 (3)**: §5 결재선 지정 규칙 추가 (`_결정대기.md` #3 확정). 자동 매칭 폐기, 상신 시점 명시적 지정, 대리자 선택 자유.
