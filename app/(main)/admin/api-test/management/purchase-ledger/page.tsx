...
export default function PurchaseLedgerTestPage() {
  ...
  const handleSearch = () => {
    ...
    if (filter.vendorCompany) params.append('vendorCompany', filter.vendorCompany)
    if (filter.ledgerType) params.append('ledgerType', filter.ledgerType)
    if (filter.paymentStatus) params.append('paymentStatus', filter.paymentStatus)
    if (filter.startDate) params.append('startDate', filter.startDate)
    if (filter.endDate) params.append('endDate', filter.endDate)
    if (filter.search) params.append('search', filter.search)
    callApi('GET', `/api/management/purchase-ledger?${params.toString()}`)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">매입장 테스트</h1>
          <p className="text-gray-500 mt-1">매입 거래 내역 등록 및 조회</p>
        </div>
        <Link
          href="/admin/api-test"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">매입장 등록</h3>
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={() => callApi('POST', '/api/management/purchase-ledger', form)}
              disabled={loading || !form.vendorCompany || !form.itemName}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">유형</label>
                <select
                  value={filter.ledgerType}
                  onChange={(e) => setFilter({ ...filter, ledgerType: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                >
                  <option value="">전체</option>
                  <option value="INVOICE">세금계산서</option>
            ...
            <button
              ...
              disabled={loading}
              className="w-full px-4 py-2 bg-gray-900 text-white rounded-lg text-sm hover:bg-gray-800 disabled:opacity-50"
            >
              조회
            </button>
          </div>
          ...
          {result.error ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div>
                  <div className="text-sm text-gray-500">부가세</div>
                  <div className="text-lg font-bold">{formatNumber(result.summary.totalVatAmount)}원</div>
                </div>
                <div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-3 py-2 text-left">계산서일</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {result.items.map((item) => (
                        <tr key={item.id} className="hover:bg-gray-50">
                          <td className="px-3 py-2">{item.invoiceDate.split('T')[0]}</td>
                          <td className="px-3 py-2">{item.vendorCompany}</td>
                          <td className="px-3 py-2">
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs">
                              {getLedgerTypeLabel(item.ledgerType)}
                            </span>
                          </td>
                          <td className="px-3 py-2">
                            <span className="px-2 py-0.5 bg-gray-100 rounded text-xs">{item.category}</span>
                          </td>
                          <td className="px-3 py-2 max-w-xs truncate">{item.itemName}</td>
                          ...
                          <td className="px-3 py-2 text-center">
                            <span className={`px-2 py-0.5 rounded text-xs ${
                              ...
                              {item.paymentStatus === 'PENDING' ? '미결제' :
                               item.paymentStatus === 'COMPLETED' ? '완료' : item.paymentStatus}
                            </span>
                          </td>
                        </tr>
                      ))}
