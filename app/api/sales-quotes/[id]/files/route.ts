...
// GET /api/sales-quotes/[id]/files - 파일 목록 조회
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    const { id } = await params

    const files = await prisma.salesQuoteFile.findMany({
      where: { quoteId: id },
      include: {
...
// POST /api/sales-quotes/[id]/files - 파일 업로드
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (quote.status === 'DRAFT') {
      return NextResponse.json(
        { error: '발송 후에만 원본 파일을 업로드할 수 있습니다' },
        { status: 400 }
      )
    }
    ...
    const s3Key = generateS3Key('sales-quotes', id, file.name)
    ...
    const savedFile = await prisma.salesQuoteFile.create({
      data: {
        quoteId: id,
