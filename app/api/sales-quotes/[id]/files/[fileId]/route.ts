...
// GET /api/sales-quotes/[id]/files/[fileId] - 파일 다운로드 URL 조회
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    const { id, fileId } = await params

    const file = await prisma.salesQuoteFile.findFirst({
      where: { id: fileId, quoteId: id },
    })
...
// DELETE /api/sales-quotes/[id]/files/[fileId] - 파일 삭제
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // DB에서 파일 정보 삭제
    await prisma.salesQuoteFile.delete({ where: { id: fileId } })
