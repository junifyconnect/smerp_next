...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. 먼저 동적 템플릿 파서 시도
    let parsed = await parseWithDefaultTemplate(
      buffer,
      "SALES_APPROVAL",
      prisma,
    );
    ...
    // 2. 템플릿이 없으면 기본 파서 사용
    if (!parsed) {
      console.log("템플릿이 없어 기본 파서 사용");
      parsed = await parseExcel(buffer, "SALES_APPROVAL");
    } else {
      console.log("동적 템플릿 파서 사용");
    }
    ...
    const parsedPurchaseItems = (parsed.purchaseItems ||
      []) as ParsedPurchaseItemWithDetails[];
    ...
    const isConsolidatedPurchase =
      vendorSet.size === 1 && parsedPurchaseItems.length > 1;
    const consolidatedVendor = isConsolidatedPurchase
      ? Array.from(vendorSet)[0]
      : "";

    // 통합 매입 시 메인 행의 매입 정보 찾기 (단가가 있는 품목)
    // 파서에서 메인 행은 마지막에 push되므로, 단가가 있는 항목을 찾음
    const mainPurchaseItem = parsedPurchaseItems.find(
    ...
    const salesItems = parsedItems.map((item, index) => {
      ...
      const details =
        item._details && item._details.length > 0
    ...
    const purchaseItems = parsedPurchaseItems.map((item, index) => {
      ...
      return {
        ...
        quantity: qty,
        unitPrice: price,
        purchaseDate: isValidDate(item.purchaseDate)
