...
// PATCH /api/management/sales-invoice-status/[id]
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('매출 계산서 업데이트 오류:', error)
...
// DELETE /api/management/sales-invoice-status/[id]
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    const { id } = await params
    await prisma.salesInvoiceStatus.delete({ where: { id } })
    return NextResponse.json({ message: '삭제되었습니다' })
  } catch (error) {
    console.error('매출 계산서 삭제 오류:', error)
    return NextResponse.json(
      { error: '삭제에 실패했습니다' },
      { status: 500 }
    )
