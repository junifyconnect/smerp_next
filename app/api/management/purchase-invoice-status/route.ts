...
export async function GET(request: NextRequest) {
  try {
    ...
    const [items, total, aggregations] = await Promise.all([
      ...
      prisma.purchaseInvoiceStatus.count({ where }),
...
// POST /api/management/purchase-invoice-status - 매입 계산서 발행현황 등록
export async function POST(request: NextRequest) {
  try {
    ...
    // 필수값 검증
    if (!itemName || !vendorCompany) {
      return NextResponse.json(
        { error: '품목명과 매입처는 필수입니다' },
        { status: 400 }
      )
    }
    ...
    const status = await prisma.purchaseInvoiceStatus.create({
      data: {
        approvalCode,
        salesApprovalId,
        maApprovalId,
    ...
    console.error('매입 계산서 발행현황 등록 오류:', error)
    return NextResponse.json(
      { error: '매입 계산서 발행현황 등록에 실패했습니다' },
      { status: 500 }
    )
  }
}
