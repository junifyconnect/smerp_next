...
// POST /api/management/purchase-invoice-status/[id]/payment - 결제내역 추가
export async function POST(
  ...
  try {
    ...
    // 기존 계산서 발행현황 조회
    const invoice = await prisma.purchaseInvoiceStatus.findUnique({
      where: { id },
    })
    ...
    // 결제내역 생성
    const payment = await prisma.purchasePaymentHistory.create({
      data: {
        purchaseInvoiceStatusId: id,
    ...
    // 계산서 발행현황 업데이트
    const updatedInvoice = await prisma.purchaseInvoiceStatus.update({
      where: { id },
      data: {
...
// DELETE /api/management/purchase-invoice-status/[id]/payment - 결제내역 삭제
export async function DELETE(
  ...
  try {
    ...
    // 결제내역 조회
    const payment = await prisma.purchasePaymentHistory.findUnique({
      where: { id: paymentId },
    })

    if (!payment || payment.purchaseInvoiceStatusId !== id) {
      return NextResponse.json(
    ...
    // 결제내역 삭제
    await prisma.purchasePaymentHistory.delete({
      where: { id: paymentId },
    })
