...
// GET /api/management/invoice-status - 계산서 발행현황 목록 조회
export async function GET(request: NextRequest) {
  try {
    ...
    const [items, total, aggregations] = await Promise.all([
      ...
      prisma.invoiceStatus.count({ where }),
      prisma.invoiceStatus.aggregate({
        where,
        _sum: {
          totalPrice: true,
          batchTotal: true,
    ...
    console.error('계산서 발행현황 목록 조회 오류:', error)
    return NextResponse.json(
...
// POST /api/management/invoice-status - 계산서 발행현황 등록
export async function POST(request: NextRequest) {
  try {
    ...
    const status = await prisma.invoiceStatus.create({
      data: {
        approvalCode,
        salesApprovalId,
        maApprovalId,
    ...
    console.error('계산서 발행현황 등록 오류:', error)
    return NextResponse.json(
      { error: '계산서 발행현황 등록에 실패했습니다' },
      { status: 500 }
    )
  }
}
