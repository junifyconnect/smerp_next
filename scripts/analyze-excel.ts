  ...
  if (typeof value === "object") {
    ...
    if ("result" in value && value.result !== undefined) {
      ...
      if (value.result instanceof Date)
        return isValidDate(value.result)
          ? value.result.toISOString().split("T")[0]
          : String(value.result);
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log(
      "예시: npx tsx scripts/analyze-excel.ts ./documents/견적서.xlsx",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    for (const sheet of analysis.sheets) {
      ...
      if (sheet.mergedCells.length > 0) {
        console.log(
          `  ${sheet.mergedCells.slice(0, 10).join(", ")}${sheet.mergedCells.length > 10 ? "..." : ""}`,
        );
      }
      ...
      console.log("\n미리보기 (처음 15행):");
      console.log(
        "    | " + sheet.previewHeaders.map((h) => h.padEnd(12)).join("| "),
      );
